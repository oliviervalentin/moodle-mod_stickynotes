...
// echo "COL ! ".$stickyid->stickycolid;
        ...
        if (isset($stickyid->edit)) {
			...
			$createorder[1000] = 'A la fin de la colonne';	
			
			foreach ($dbresult as $move) {
				$neworder = $move->ordernote+1;
				$createorder[$neworder] = $move->message;
			}
			
			$mform->addElement('select', 'ordernote', 'Ordre', $createorder);
            $mform->setType('ordernote', PARAM_INT);
            // $mform->setDefault('ordernote', 0);  
print_object($createorder);
...
// $data->ordernote = 'TAMERE';
// print_object($data);exit();
	 // Check ordernote : if not set, user creates note at the end of the column.
	if($data->ordernote == 1000) {
		echo "ZERO";exit();
		// Check last note order and add 1 to define place of the new note.
	...
	// If order note is set, user creates a note at a specific place.
	// Ordernote is set in the previous form.
	else {
		echo "numero ".$data->ordernote;
		print_object($data); exit();
		}
